...
function getQueues(req, res) {
    try {
        ...
        if (companyid) {
            if (catalog && type) {
                params = {toCompanyid: companyid, catalog: catalog, type: type}
            } else if (catalog) {
                params = {toCompanyid: companyid, catalog: catalog}
            } else if (type) {
                params = {toCompanyid: companyid, type: type}
            } else {
        ...
        //获取消息，先进先出
        queueDao.intervalQueue(queueName, params).then(function (data) {
            if (data) {
...
router.put("/", function (req, res) {
    try {
        //校验数据
        vali.validateData(req, res).then(function (v) {
            var queueName;
            if (curr_queue) {
                queueName = curr_queue;
            } else {
...
router.get('/getQueuesByCid/:cid', function (req, res, next) {
    try {
        ...
        queueDao.getQueuesByCid(queueName, {clientReference: cid}).then(function (data) {
...
router.get('/getQueueByOid/:oid', function (req, res, next) {
    try {
        ...
        if(oid){
            queueDao.getQueueByOid(queueName, oid).then(function (data) {
                if (data) {
                    util.sendJSONresponse(res, 200, data);
                } else {
                    util.sendJSONresponse(res, 200, {"message": "没找到数据"});
